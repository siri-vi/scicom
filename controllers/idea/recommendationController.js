...
const {sendNotif} =require('../common/Notifs')
    ...
    try {
        ...
        if(result && result.length!==0){
            ...
            if(result1 && result1!==0){
                ...
                sdk['add-friend']({accepted: [`${target_id}`]},{uid: user_id, apiKey: process.env.cometchat_api_key})
                ...
                if(action==="poke"){
                    if(pokesLeft>0){
                        ...
                        const pokesData = await User.findByIdAndUpdate({user_id:user_id},pokesLeft);
                        const like = await Likes.create({user_id, target_id, user1, user2, action })
                        res.status(200).json({
                                status:"success",
                                like,
